...
/*
    public function returnSpotCurr() {
        $currentNordpoolFile = $this->config['textbase'] . 'spotprice.txt';
        $filearrayCurrent = file($currentNordpoolFile);
        return $filearrayCurrent;
    }

    public function returnSpotNew() {
    ...
    */ 
    ...
    public function LoadLists(){
		...
		if ($this->tomorrowsDate === $now) {
    		$newExtractedList = $this->config['textbase'] . 'newPrices.txt';
    		$newList = file_get_contents($newExtractedList);
    		$currentExtractedList = $this->config['textbase'] . 'currentPrices.txt';
    		file_put_contents($currentExtractedList, $newList);

            $newNPFile = $this->config['textbase'] . 'spotprice2.txt';
            $newSpotFile = file_get_contents($newNPFile);
            $currentNPFile = $this->config['textbase'] . 'spotprice.txt';
            file_put_contents($currentNPFile, $newSpotFile);
		}
        ...
        if(isset($tomclean) && null !== $tomclean){ 
            $extract2 = array();
            $extract2 = $this->nonzero($tomclean);  
		    $max2 = max($extract2);
            $min2 = min($extract2);
        }

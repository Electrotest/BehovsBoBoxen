        ...
        switch ($action) {
            case 'install':
                try {
                    ...
                    $this->db->ExecuteQuery(self::SQL('insert translate'), array('Loadcontrol', 'DSM'));
...
$this->db->ExecuteQuery(self::SQL('insert translate'), array('The BehovsBoMethod','BehovsBoMetoden'));
$this->db->ExecuteQuery(self::SQL('insert translate'), array('
The BehovsBoMethod makes houses clever in Nordic climate.','BehovsBoMetoden gör hus smarta i nordiska klimat.'));
$this->db->ExecuteQuery(self::SQL('insert translate'), array('The BehovsBoMethod is a controlsystem for heating of your house.','BehovsBoMetoden är ett styrsystem för uppvärmningen av ditt hus.'));
$this->db->ExecuteQuery(self::SQL('insert translate'), array('Why','Varför'));
$this->db->ExecuteQuery(self::SQL('insert translate'), array('Nordpool sets the electricity-spotprice per hour depending of acess and demand. ','Nordpool sätter idag elspotpriset per timma beroende på tillgång och efterfrågan. '));
$this->db->ExecuteQuery(self::SQL('insert translate'), array('In wintertime when the energydemand is high in the society the price can rise several hundred percent. ','På vintern när energibehovet är stort i samhället kan priset sticka iväg flera hundra procent. '));
$this->db->ExecuteQuery(self::SQL('insert translate'), array('When everybody takes a shower and cookes food at about the same time, the need of expensive toppower gets high. ','När alla duschar och lagar mat ungefär samtidigt blir behovet av dyr toppkraft stor. '));
$this->db->ExecuteQuery(self::SQL('insert translate'), array('Then the sudden outtake of electricty harm the grids. ','Då kan plötsliga samtidiga uttag av el skada distributionsnäten. '));
$this->db->ExecuteQuery(self::SQL('insert translate'), array('Everybody wins when the private households controls the outtake, the society (fewer powercompanies), the local society (safer grids), and peoples private economy.','Alla vinner på att privata hushåll hanterar effektuttaget, samhället (färre kraftverk), lokalsamhället (tryggare nät) och privatpersoner (ekonomi). '));
$this->db->ExecuteQuery(self::SQL('insert translate'), array('On', "På"));
$this->db->ExecuteQuery(self::SQL('insert translate'), array('Off', 'Av'));
//$this->db->ExecuteQuery(self::SQL('insert translate'), array('',''));

...
class CInterceptionFilter extends CObject implements ISingleton {
    ...
    public function AdminOrForbidden($msg = null) {
        ...
        if (!$this->user->IsAdmin()) {
            $this->bbb->ShowErrorPage(403, $msg);
        }
        return $this;
    }
    ...
    public function MatchSecretKey($secret = null) {
        if ($this->config['secret_key'] != $secret) {
            $this->bbb->ShowErrorPage(403, t('You need admin-privileges to access this page.'));
        }
        return $this;
    }
    ...
    public function CreateNewUserOrForbidden() {
        if (!$this->config['create_new_users']) {
            $this->bbb->ShowErrorPage(403, t('Create new user is disabled by site owner.'));
        }
        return $this;
    }
    ...
    public function IsRegularUserOrForbidden() {
        if (!$this->user->IsUser()) {
            //echo __DIR__;
            //var_dump($this);
            $this->bbb->ShowErrorPage(403, t('You are just visiting this site and have no profile.'));
        }
        return $this;
    }
    ...
    public function AnonomousToForbidden() {
        if ($this->user->IsAnonomous()) {
            $this->bbb->ShowErrorPage(403, t('You need to login to access this content.'));
        }
        return $this;
    }

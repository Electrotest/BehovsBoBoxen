...
class CMTemperatures extends CObject implements IHasSQL, ArrayAccess, IModule {
    ...
    public static function SQL($key = null, $args = null) {
        ...
        $queries = array(
            ...
            'create table roomsettings' => "CREATE TABLE IF NOT EXISTS roomsettings (id INTEGER PRIMARY KEY NOT NULL, room TEXT, home DOUBLE, max DOUBLE, min DOUBLE, away DOUBLE, rund DOUBLE, fromdate DATETIME, todate DATETIME);",
            'insert roomsettings' => "INSERT INTO roomsettings (room, home, max, min, away, rund) VALUES (?,?,?,?,?,?);",
            'select all' => "SELECT id, room, home, max, min, away, rund, fromdate, todate FROM roomsettings;",
            'update roomsettings' => "UPDATE roomsettings SET home = ?,max = ?, min = ?, away = ?, rund = ?, room = ? WHERE id = ?;",  
            'update roomname' => "UPDATE roomsettings SET room = ? WHERE id = ?;",
            'select * by room' => "SELECT id, home, max, min, away, rund, fromdate, todate FROM roomsettings WHERE room = ?",
     ...
     * @returns boolean true if success else false.
     */
    public function Update($home, $max, $min, $away, $rund, $room, $id) {
        $msg = null;
        if ($room) {
            $this->db->ExecuteQuery(self::SQL('update roomsettings'), array($home, $max, $min, $away, $rund, $room, $id));
            $msg = 'updated';
        } 
